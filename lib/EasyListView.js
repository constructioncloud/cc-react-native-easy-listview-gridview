...
export default class EasyListView extends Component {
    ...
    constructor(props) {
        ...
        const ds = new ListView.DataSource({rowHasChanged: (prevRowData, nextRowData) => prevRowData !== nextRowData, sectionHeaderHasChanged: (s1, s2) => s1 !== s2});
        this.state = {
            pageNo: 1,
            rawData: [],
            dataSource: ds,
    ...
    _onSuccess(dataBlob, sectionIDs, rowIDs) {
        var newData = rowIDs instanceof Array ? rowIDs : JSON.parse(rowIDs)
        ...
        this.setState({
            ...
            rawData: allData,
            dataSource: this._getDataSource(allData, sectionIDs, rowIDs),
            isFirstLoad: false,
            isRefreshing: false,
            isLoadingMore: false,
            isError: false,
            isEmpty: allData.length === 0,
            loadMoreEnable:  newData.length !== 0 //newData.length >= this.props.dataSizePerPage // newData.length !== 0
        })
    }
    ...
    _getDataSource(dataBlob, sectionIDs, rowIDs) {
        // return  this.state.dataSource.cloneWithRowsAndSections(this.props.column === 1 ? this.props.responseDataHandler(dataBlob, sectionIDs, rowIDs) : this._groupItems(this.props.responseDataHandler(dataBlob), sectionIDs, rowIDs))
        return  this.state.dataSource.cloneWithRowsAndSections(this.props.responseDataHandler(dataBlob, sectionIDs, rowIDs))
        /*
        this.state.dataSource.cloneWithRows(this.props.column === 1
    ...
    // GridView
    _renderGroup(dataBlob, sectionIDs, rowIDs) {
        var itemsGroups = []
        itemsGroups.push(dataBlob)

        var items = itemsGroups.map((item, index) => {
    ...
    /*
    _groupItems(items, sectionIDs, rowIDs) {
        var itemsGroups = []
        var group = []

        alert(this.props.column)
        alert(group.length)
